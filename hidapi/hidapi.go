...
// #cgo LDFLAGS: -Wl,--allow-multiple-definition -L${SRCDIR} -lsetupapi -lhid -lhidapi
// #include <stdlib.h>
// #include "hidapi.h"
// #include "hid.c"
import "C"
...
// c2goCopy copies a C uchar buffer into a Go []byte slice.
func c2goCopy(dst []byte, src *C.uchar) []byte {
	x := (*[1 << 30]byte)(unsafe.Pointer(src))
	copy(dst, x[:])
	return dst
}

// go2cCopy copies a Go []byte slice to a C uchar buffer.
func go2cCopy(dst *C.uchar, src []byte) *C.uchar {
	x := (*[1 << 30]byte)(unsafe.Pointer(dst))
	copy(x[:], src)
	return dst
}

// c2goSlice creates a Go []byte slice and copies in a C uchar buffer.
func c2goSlice(buf *C.uchar, n int) []byte {
	s := make([]byte, n)
	return c2goCopy(s, buf)
}

// allocBuffer allocates a C uchar buffer of length n bytes.
// Call freeBuffer on the returned C buffer.
func allocBuffer(n int) *C.uchar {
	return (*C.uchar)(C.malloc(C.size_t(n)))
}

// freeBuffer frees a C uchar buffer.
func freeBuffer(buf *C.uchar) {
	C.free(unsafe.Pointer(buf))
}

// Set a value within a C uchar buffer.
// func (buf *C.uchar) set(i int, val byte) {
...
func Enumerate(vid, pid uint16) []*DeviceInfo {
	diList := C.hid_enumerate(C.ushort(vid), C.ushort(pid))
	if diList == nil {
		return nil
	}
	devs := []*DeviceInfo{}
...
func Open(vid, pid uint16, sn string) (*Device, error) {
	...
	if sn == "" {
		dev = C.hid_open(C.ushort(vid), C.ushort(pid), nil)
	} else {
		ws, err := wchar.FromGoString(sn)
		if err != nil {
			return nil, errors.New("can't convert serial number")
		}
		dev = C.hid_open(C.ushort(vid), C.ushort(pid), (*C.wchar_t)(ws.Pointer()))
	}
	if dev == nil {
...
func (d *Device) Write(data []byte) error {
	...
	rc := int(C.hid_write(d.dev, cBuffer, C.size_t(cLength)))
	if rc < 0 {
...
func (d *Device) ReadTimeout(id byte, length, milliseconds int) ([]byte, error) {
	...
	defer freeBuffer(cBuffer)
	// cBuffer.set(0, id)
	x := (*[1 << 30]byte)(unsafe.Pointer(cBuffer))
	x[id] = 0

	rc := int(C.hid_read_timeout(d.dev, cBuffer, C.size_t(cLength), C.int(milliseconds)))
	if rc < 0 {
...
func (d *Device) Read(id byte, length int) ([]byte, error) {
	...
	rc := int(C.hid_read(d.dev, cBuffer, C.size_t(cLength)))
	if rc < 0 {
...
func (d *Device) SendFeatureReport(data []byte) error {
	...
	defer freeBuffer(cBuffer)
	go2cCopy(cBuffer, data)
	rc := int(C.hid_send_feature_report(d.dev, cBuffer, C.size_t(cLength)))
	if rc < 0 {
...
func (d *Device) GetFeatureReport(id byte, length int) ([]byte, error) {
	...
	rc := int(C.hid_get_feature_report(d.dev, cBuffer, C.size_t(cLength)))
	if rc < 0 {
