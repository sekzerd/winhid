...
#define WSTR_LEN 512
            ...
            BOOLEAN res;
            NTSTATUS nt_res;
            wchar_t wstr[WSTR_LEN] = {0}; /* TODO: Determine Size */
            size_t len;
...
hid_device* hid_open_path(const char *path)
{
    ...
    HIDD_ATTRIBUTES attrib;
    HidD_GetAttributes(dev->device_handle, &attrib);
    dev->device_version_number = attrib.VersionNumber;
    return dev;
...
{
    BOOL res = HidD_SetOutputReport(dev->device_handle,data,length);
    return res ? 0: -1;
    // if (!res)
    // {
    //     return -1;
...
{
#if 0
    BOOL res = HidD_GetInputReport(dev->device_handle,data,length);
#endif
    DWORD bytes_returned;

    OVERLAPPED ol;
    memset(&ol, 0, sizeof(ol));
    return DeviceIoControl(dev->device_handle,
